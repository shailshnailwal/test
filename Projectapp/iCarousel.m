...
{
    CGFloat f=[self minScrollDistanceFromOffset:_scrollOffset toOffset:offset];
    [self scrollByOffset:f duration:duration];
}
...
- (void)didTap:(UITapGestureRecognizer *)tapGesture
{
    ...
    if (index != NSNotFound)
    {
        if (!_delegate || [_delegate carousel:self shouldSelectItemAtIndex:index])
        {
            ...
            if ((index == self.currentItemIndex && _scrollToItemBoundary))
            {
                 [self scrollToItemAtIndex:index animated:YES];
               //[self scrollToItemAtIndex:index duration:1.5];
            }
